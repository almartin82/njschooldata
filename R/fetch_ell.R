...
#' row. The EL share of enrollment (`pct_of_enrollment`, 0-100) is computed fresh
#' from the published headcount; for the 2020-2022 district/school files, which
#' publish only a percent, that published percent is carried through and
#' `n_students` is `NA` (the count is never back-derived from the percent).
#'
...
fetch_ell <- function(end_year, tidy = TRUE, use_cache = FALSE) {
  if (!end_year %in% ELL_VALID_YEARS) {
    return(empty_ell_frame())
  }

  if (use_cache) {
    key <- make_cache_key("fetch_ell", end_year = end_year, tidy = tidy)
...
fetch_ell_multi <- function(end_years, tidy = TRUE, use_cache = FALSE) {
  ...
  purrr::map_df(
    sort(available),
    function(.y) fetch_ell(.y, tidy = tidy, use_cache = use_cache)
  )
}

...
* `fetch_reportcard_special_pop()` reads special population data from report card files
...
* `fetch_enr()` aggregates gender subgroups into racial subgroups (white_m + white_f = white)
...
* Ability to read in Taxpayers Guide to Educational Spending data files with `get_raw_tges()` and `fetch_tges()`

# njschooldata 0.5.2

## New features

* Ability to read in Federal ESSA (Every Student Succeeds Act) accountability data files with `get_essa_file()`.
...
* Support for PARCC data via the `fetch_parcc()` family of functions.
